...
FEEDBACK_CANDIDATES = DATA_DIR / "intent_rag_feedback_candidates.jsonl"
FEEDBACK_APPROVED = DATA_DIR / "intent_rag_feedback_approved.jsonl"
...
def build_from_approved_feedback() -> list[dict[str, Any]]:
    """Load human-approved QA feedback examples.

    User ratings never enter the corpus directly.  A row must be marked
    `review_status: approved`, either in the candidates file after review or
    in `intent_rag_feedback_approved.jsonl`.
    """
    rows = _load_jsonl(FEEDBACK_CANDIDATES) + _load_jsonl(FEEDBACK_APPROVED)
    records: list[dict[str, Any]] = []
    for row in rows:
        if str(row.get("review_status") or "").lower() != "approved":
            continue

        approved = row.get("approved_record") or row.get("suggested_record") or row
        text = str(approved.get("text") or row.get("question") or "").strip()
        if not text:
            continue
        labels = [str(label) for label in (approved.get("intent_labels") or []) if str(label).strip()]
        if not labels:
            labels, _, _ = _infer_labels(text)
        sub_intents = [str(s) for s in (approved.get("sub_intents") or []) if str(s).strip()]
        answer_mode = str(approved.get("answer_mode") or "router_assist")
        keywords = [str(k) for k in (approved.get("keywords") or []) if str(k).strip()]
        metadata = dict(approved.get("metadata") or {})
        metadata.update({
            "qa_feedback_review_status": "approved",
            "candidate_id": row.get("candidate_id"),
            "qa_log_id": row.get("qa_log_id") or metadata.get("qa_log_id"),
            "candidate_reasons": row.get("candidate_reasons") or [],
        })
        records.append(_record(
            record_id=str(approved.get("id") or f"qa_feedback:{row.get('candidate_id') or len(records)}"),
            source_type="qa_feedback_approved",
            source_file="intent_rag_feedback_candidates.jsonl",
            title=str(approved.get("title") or row.get("question") or "QA feedback approved")[:120],
            text=text,
            keywords=keywords,
            intent_labels=labels,
            sub_intents=sub_intents,
            answer_mode=answer_mode,
            weight=float(approved.get("weight") or 0.82),
            metadata=metadata,
        ))
    return records
...
def main() -> None:
    groups = {
        ...
        "qa_feedback_approved": build_from_approved_feedback(),
    }
