...
function buildRouteNarrative(payload, primary, summary, checks) {
  ...
  if (routeMode !== "goods") {
    ...
    addLine(
      ...
      isService
        ? "용역 계약은 물품식별번호보다 과업지시서의 업무 범위, 요구 업종·면허, 인력·장비 요건, 수행실적, 지역제한 가능 여부, 발주기관 적용 법령을 먼저 확인해야 합니다."
        : "공사는 공고의 공종, 요구 면허, 주력분야, 시공능력평가금액, 현장소재지, 지역제한 입찰 가능 여부, 공동도급 가능 여부를 먼저 확인해야 합니다.",
      "route",
    );
    addLine(
      "업체 현황",
