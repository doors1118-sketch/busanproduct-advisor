...
PILOT_AUTH_PASSWORD=pilot_pass_123!
...
"""
    ...
    # Clean previous logs
    ssh.exec_command("rm -f /var/log/busan_advisor_pilot_out.log /var/log/busan_advisor_pilot_err.log")
...
Type=simple
User=root
WorkingDirectory=/root/e2e_workspace
EnvironmentFile=/root/e2e_workspace/.env
EnvironmentFile=/root/e2e_workspace/pilot_auth.env
...
"""
    ...
    stdin, stdout, stderr = ssh.exec_command("cat /var/log/busan_advisor_pilot_err.log")
    err_log = stdout.read().decode()
    if err_log:
        print("ERR LOG:", err_log)
...
env = {}
with open("/root/e2e_workspace/pilot_auth.env", "r", encoding="utf-8") as f:
    for line in f:
        line = line.strip()
