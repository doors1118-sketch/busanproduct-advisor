...
from app.runtime.company_api_adapter import CompanyAPIAdapter, CompanyCandidateResult, _default_use_mock
...
class ChatbotRuntimeOrchestrator:
    def __init__(self, use_mock_company_api: bool = None):
        self.router = GeminiIntentRouter()
        if use_mock_company_api is None:
            use_mock_company_api = _default_use_mock()
    ...
    def run(self, request: ChatbotRuntimeRequest) -> ChatbotRuntimeResponse:
        ...
        # ── Company API 실패 시 사용자 안내 문구 준비 ──
        company_fail_notice = None
        if company_result and company_result.status == "failed":
            error_type = getattr(company_result, 'error_type', None) or 'unknown'
            if error_type == "timeout":
                company_fail_notice = "⚠️ 업체 조회 서버 응답 지연으로 후보업체 목록을 표시할 수 없습니다. 잠시 후 다시 시도해 주세요."
            elif error_type == "connection":
                company_fail_notice = "⚠️ 업체 조회 서버에 연결할 수 없어 후보업체 목록을 표시할 수 없습니다."
            else:
                company_fail_notice = "⚠️ 업체 조회가 일시적으로 불가하여 후보업체 목록을 표시할 수 없습니다."
        ...
        try:
            ...
            if company_fail_notice and answer_output.rendered_markdown:
                ...
                if caution_marker in answer_output.rendered_markdown:
                    answer_output.rendered_markdown = answer_output.rendered_markdown.replace(
                        caution_marker,
                        notice_block + caution_marker
                    )
                else:
                    answer_output.rendered_markdown += notice_block

            # 중복 파이프라인 방지: Final Forbidden Phrase Scan은 하단에서 통합 처리
