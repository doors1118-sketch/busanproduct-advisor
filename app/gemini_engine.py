...
def _build_grounded_case_timeout_fallback(user_message: str) -> str:
    ...
    return "\n".join([
        "내부 DB 근거는 조회되었지만 LLM 문장화가 지연되어 답변을 간단히 유보합니다.",
...
def _build_simple_amount_contract_answer(user_message: str, amount_detected) -> str | None:
    ...
    if amount_detected >= 200_000_000:
        return "\n".join([
            "### 판단 요약",
            "- 질문 조건이 **지방자치단체 기준의 물품 2억원**이라면, 일반적인 소액 물품 수의계약 기준만으로는 해당 방식을 바로 적용하기 어렵습니다.",
            ...
            "",
            "### 지역상품 구매 지원 관점",
            "- 이 사안을 특정 업체와의 직접 계약으로 단정하기보다 **지역제한경쟁입찰**, **종합쇼핑몰/MAS**, **중소기업자간 경쟁제품·직접생산확인**, **기술개발제품·혁신제품·우수조달물품** 여부를 함께 검토하는 방향이 안전합니다.",
