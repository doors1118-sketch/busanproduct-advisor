...
def _local_supplier_numeric_labels() -> dict[str, str]:
    ...
    return {
        ...
        "local_general_construction": (
            get_numeric_display("P_LOCAL_LIMITED_BID_GENERAL_CONSTRUCTION_THRESHOLD")
            or "종합공사 지역제한 기준 확인 필요"
        ),
        "local_specialty_construction": (
            get_numeric_display("P_LOCAL_LIMITED_BID_SPECIALTY_CONSTRUCTION_THRESHOLD")
            or "전문공사 지역제한 기준 확인 필요"
        ),
        "local_technical_service": (
            get_numeric_display("P_LOCAL_LIMITED_BID_TECHNICAL_SERVICE_THRESHOLD")
            or "기술용역 지역제한 기준 확인 필요"
        ),
        "local_joint_min_share": get_numeric_display("P_LOCAL_JOINT_CONTRACT_MIN_SHARE") or "공동도급 최소비율 확인 필요",
        "local_joint_max_share": get_numeric_display("P_LOCAL_JOINT_CONTRACT_MAX_SHARE") or "공동도급 상한비율 확인 필요",
    }


def _bid_notice_local_company_lines() -> list[str]:
    labels = _local_supplier_numeric_labels()
    return [
        "### 입찰공고문 지역업체 활용 조건 설계",
        "- 입찰공고문은 법적 효력이 있는 문서이므로 `부산업체를 돕겠다`는 목적보다 **계약유형, 추정가격, 참가자격, 낙찰자 결정방법, 평가항목 근거**를 먼저 맞춰야 합니다.",
        "- 지역업체 활용 조건은 크게 `지역제한`, `지역의무 공동도급`, `지역업체 참여도·가점`, `현장 대응성·납품/A/S 평가항목` 중 무엇인지 구분해서 써야 합니다.",
        "- 금액·비율·점수는 공고문에 바로 하드코딩하지 말고, 최신 법령·고시·기관 자체기준을 확인한 뒤 기재합니다.",
        "",
        "### 금액·계약유형별로 쓸 수 있는 장치",
        "| 구분 | 공고문에 쓸 수 있는 방식 | 기준 확인 지점 | 실무 판단 |",
        "|---|---|---|---|",
        f"| 물품·일반용역 | 지역제한 또는 2인 이상 견적 지역제한 검토 | 시·도 기준은 **{labels['local_general_service']}**, 부산 관할 군·구 기준은 **{labels['local_busan_gu_gun']}** 확인 | 기준 안이면 `부산광역시 소재 업체` 제한을 검토하고, 기준을 넘으면 지역제한 대신 납기·A/S·현장지원 평가항목으로 설계합니다. |",
        f"| 기술용역 | 지역제한, 지역업체 참여도, 공동수급 검토 | 기술용역 지역제한 기준 **{labels['local_technical_service']}**, 낙찰자 결정기준 별표 | 기술·학술용역의 지역업체 참여도 점수는 해당 별표가 맞는지 확인한 뒤 적용합니다. 일반용역에 그대로 가져오면 안 됩니다. |",
        f"| 종합공사 | 지역제한 또는 지역의무 공동도급 검토 | 종합공사 지역제한 기준 **{labels['local_general_construction']}** | 기준을 넘으면 부산업체만 입찰하도록 제한하기보다 지역의무 공동도급·지역업체 참여도 평가를 검토합니다. |",
        f"| 전문공사·그 밖의 공사 | 지역제한, 면허·주력분야, 공동도급 검토 | 전문공사 등 지역제한 기준 **{labels['local_specialty_construction']}** | 공종·면허가 맞는 부산업체 풀이 충분한지 시장조사 후 지역제한을 넣습니다. |",
        f"| 대형·복합 사업 | 공동수급·분담이행, 지역업체 참여계획 평가 | 공동도급 비율은 기관 기준과 예규에서 확인. 내부 기준 예시는 **{labels['local_joint_min_share']}~{labels['local_joint_max_share']}** 범위 확인 | 전국 업체 참여가 불가피하면 부산업체 지분, 담당 과업, 투입인력, 책임범위를 평가하도록 설계합니다. |",
        "",
        "### 참가자격 문구: 넣을 문구와 빼야 할 문구",
        "| 항목 | 위험한 문구 | 권장 문구 | 이유 |",
        "|---|---|---|---|",
        "| 소재지 | `부산에 지사·연락소가 있는 업체` | `입찰공고일 전일부터 입찰일까지 주된 영업소의 소재지가 부산광역시에 있는 업체. 낙찰자는 계약체결일까지 이를 유지하여야 한다.` | 지역제한은 본점·주된 영업소 기준을 명확히 해야 분쟁이 줄어듭니다. |",
        "| 실적 | `부산시 또는 부산 소재 기관 수행실적 보유 업체` | `최근 ○년 이내 유사 규모·유사 난이도 사업 수행실적` | 실적은 과업 수행능력 기준으로 쓰고, 특정 지역 실적만 요구하면 부당제한 위험이 큽니다. |",
        "| 장비·인력 | `공고일 현재 부산 내 장비·창고·상주인력 보유` | `계약체결 후 착수 전까지 현장 대응 인력·장비 확보계획 제출` | 공고일 현재 보유요건은 특정 업체 맞춤 조건으로 보일 수 있습니다. |",
        "| 특정업체·브랜드 | `특정 거래처 경험`, `특정 제품만 납품 가능` | `과업 수행에 필요한 성능·규격·인증·A/S 기준` | 지역업체 지원 목적이 특정업체 특혜로 보이면 감사 리스크가 큽니다. |",
        "| 공동수급 | `부산업체 명목 참여만 허용` | `부산업체의 참여비율, 직접 수행 과업, 투입인력, 책임범위를 제안서에 명시` | 형식적 하도급이 아니라 실질 수행 지분을 보게 해야 합니다. |",
        "",
        "### 낙찰자 결정기준·평가표 연결",
        "- **적격심사**: 공고문에 적용할 낙찰자 결정기준과 별표 번호를 명시하고, 지역업체 참여도나 신인도 항목이 해당 계약유형에 맞는지 확인합니다.",
        "- **협상에 의한 계약**: `부산업체라서 가점`이 아니라 지역 현장 운영, 긴급 대응, 납품·A/S, 민원·안전 대응, 지역 협력망처럼 과업 품질과 연결된 항목으로 설계합니다.",
        "- **물품·MAS**: 지역제한 입찰과 MAS 2단계 경쟁은 제도가 다르므로, 종합쇼핑몰 구매라면 공급업체 소재지·납품지역·사후관리 평가 가능성을 따로 봅니다.",
        "- **공사**: 지역제한, 지역의무 공동도급, 지역업체 참여도는 공종·금액·면허·공동수급 허용방식과 함께 맞춰야 합니다.",
        "",
        "### 공고 전 최종 점검",
        "- 공고문 텍스트의 지역제한 조건과 나라장터(G2B) 시스템 투찰 제한 설정이 일치하는지 대조합니다.",
        "- 지역제한과 실적제한, 면허제한, 장비보유 조건이 중복되어 경쟁을 과도하게 줄이지 않는지 확인합니다.",
        "- 시장조사표에 부산 내 경쟁 가능한 업체 수, 면허·업종, 후보 업체의 조달등록 상태, 제한 필요성을 남깁니다.",
        "- 공고 후 조건을 바꾸면 정정공고·공고기간 문제가 생기므로, 참가자격·평가기준·제출서류·입찰무효 사유를 사전에 맞춰 둡니다.",
        "",
        "정리하면, 지역업체 활용 조건은 **자격 제한으로 쓸 수 있는지 먼저 보고, 그 범위를 넘으면 평가항목·공동수급·현장 대응성으로 전환**해야 합니다. 공고문에는 정책 목적보다 법적 근거, 금액 기준, 적용 평가기준, 부당제한 방지 자료가 먼저 보여야 합니다.",
    ]
...
def _security_service_regional_lines() -> list[str]:
    labels = _local_supplier_numeric_labels()
    return [
        "### 2. 경비ㆍ무인경비 용역은 `지역제한`보다 먼저 `경비업 허가`와 과업 범위를 봐야 합니다",
        "- 청사 경비용역은 인력이 현장에 상주하고 출입통제, 순찰, 도난ㆍ화재 예방, 비상 대응을 수행하는 용역입니다.",
        "- 무인경비는 원격관제, 출동 대응, 장비 설치ㆍ연동이 섞일 수 있으므로 기계경비업ㆍ시설경비업ㆍ정보통신공사업 범위를 먼저 나눠야 합니다.",
        "- 따라서 부산업체 중심으로 검토하더라도 **부산 소재 여부보다 먼저 경비업법상 허가와 과업 범위가 맞는지** 확인해야 합니다.",
        "- 지역업체 수주 지원은 `부산업체 지정`이 아니라 **시설경비업 허가 + 지역제한 가능 금액 + 적격심사/협상계약 평가항목**을 조합하는 방식으로 설계합니다.",
        "",
        "### 3. 필수 면허ㆍ업종 설계",
        "| 과업 범위 | 우선 확인할 자격 | 공고문 설계 포인트 | 주의할 점 |",
        "|---|---|---|---|",
        "| 청사 출입통제, 순찰, 도난ㆍ화재 예방, 민원 대응 | 「경비업법」 제4조에 따른 경비업 허가 중 **시설경비업** | 나라장터 입찰참가자격에 시설경비업 등록 여부를 확인하고, 공고상 업종코드(예: 1164)와 허가증 유효성을 제출서류로 받습니다. | 경비와 무관한 시설관리ㆍ미화ㆍ주차관리 면허를 한꺼번에 요구하면 부당제한이 될 수 있습니다. |",
        "| 국가중요시설 또는 고도의 보안시설 | 특수경비업 해당 여부 별도 확인 | 청사의 법적 성격이 국가중요시설인지 먼저 확인합니다. | 일반 청사를 특수경비업으로 과도하게 제한하면 경쟁 제한 위험이 큽니다. |",
        "| 무인경비, 원격관제, 출동 대응 중심 | 기계경비업 또는 시설경비업과의 역할 분리 | 장비 관제와 현장 경비가 함께 있으면 업무 범위를 나누어 자격을 설계합니다. | 시설경비업만으로 무인경비 시스템 운영까지 모두 처리한다고 단정하지 않습니다. |",
        "| CCTVㆍ출입통제 장비 설치, 통신 배선, 관제시스템 구축 포함 | 정보통신공사업, 소프트웨어사업자 등 분리 또는 분담이행 검토 | 장비 설치ㆍ연동은 경비용역과 별도 과업인지, 분담이행 대상인지 구분합니다. | 경비용역 입찰에 정보통신공사 면허를 무조건 묶으면 과도한 복합 제한이 될 수 있습니다. |",
        "| 안내, 주차, 미화, 시설관리 업무가 섞임 | 주된 목적과 업무 지휘관계 확인 | 경비와 부대업무를 분리 발주할지, 복합용역으로 할지 검토합니다. | 경비원이 허가받은 경비업무 외 업무를 상시 수행하도록 설계하면 경비업법 리스크가 생길 수 있습니다. |",
        "",
        "### 4. 지역제한과 부산업체 수주 지원 전략",
        "| 검토 수단 | 적용 방향 | 부산업체 지원 효과 | 확인할 근거 |",
        "|---|---|---|---|",
        f"| 부산 지역제한 입찰 | 지방계약 대상 일반용역이면 추정가격이 지역제한 가능 범위인지 먼저 확인합니다. 시ㆍ도 일반용역은 **{labels['local_general_service']}**, 부산 관할 군ㆍ구 일반용역은 **{labels['local_busan_gu_gun']}** 기준을 확인합니다. | 기준 안이면 시설경비업 허가를 가진 부산업체만 경쟁하게 할 수 있어 가장 직접적입니다. | 지방계약법 시행령 제20조, 시행규칙 제24조, 행안부 고시금액 |",
        "| 2인 이상 견적 지역제한 | 소액수의 안내공고 구간이면 부산 소재 시설경비업체로 견적 제출 대상을 제한할 수 있는지 검토합니다. | 정식 입찰보다 절차 부담을 낮추면서 부산업체 견적 참여를 유도합니다. | 수의계약 사유, 견적 방식, 금액 기준 |",
        "| 공동수급ㆍ분담이행 | 규모가 크거나 CCTVㆍ출입통제 시스템이 결합되면 시설경비업체와 정보통신ㆍ관제 업체의 역할을 분담할 수 있습니다. | 전국 업체가 들어와도 부산 시설경비업체가 현장 경비 지분을 확보할 수 있습니다. | 공동계약 허용 여부, 분담 과업의 명확성 |",
        "| 협상계약 평가항목 | 고도 보안, 야간 대응, 민원ㆍ비상상황 대응 품질이 중요하면 제안서 평가를 활용합니다. | 부산업체의 현장 접근성, 긴급 인력 투입, 지역 경찰ㆍ소방 협력체계가 정성평가에서 강점이 됩니다. | 협상에 의한 계약 기준, 기관 자체 평가표 |",
        "- 국가기관ㆍ공기업ㆍ준정부기관이면 지방계약 금액 기준을 그대로 쓰지 말고, 국가계약법령ㆍ공기업ㆍ준정부기관 계약사무규칙ㆍ기관 자체 계약규정의 지역제한과 공동계약 기준을 먼저 확인합니다.",
        "",
        "### 5. 낙찰방식별 설계 포인트",
        "| 방식 | 적합한 경우 | 지역업체 반영 방법 | 주의할 점 |",
        "|---|---|---|---|",
        "| 일반용역 적격심사 | 표준적인 청사 경비, 과업이 비교적 정형화된 경우 | 지역제한 가능 여부, 경영상태, 이행실적, 신인도, 근로조건 이행계획을 공고문 기준과 맞춥니다. | 지역업체 참여도 배점은 계약유형별 별표가 다르므로 기술ㆍ학술용역 기준을 경비용역에 그대로 가져오면 안 됩니다. |",
        "| 협상에 의한 계약 | 보안 수준, 출입통제, 야간ㆍ휴일 대응, 통합관제 운영 품질이 중요한 경우 | 현장 운영계획, 비상상황 대응, 결원 대체인력, 교육훈련, 민원응대, 지역 협력체계를 평가합니다. | `부산업체라서 가점`이 아니라 수행 품질과 연결된 항목으로 설계해야 합니다. |",
        "| 최저가 중심 방식 | 단순 경비 인력 배치 위주인 경우 | 지역제한 가능 범위 안에서 경쟁풀을 부산으로 한정하는 방식이 핵심입니다. | 인건비 과소 산정, 최저임금ㆍ4대보험 누락, 경비원 교육비 누락이 생기지 않도록 산출내역을 봅니다. |",
        "",
        "### 6. 공고문 문구 예시",
        "- **참가자격**: `「경비업법」 제4조에 따른 시설경비업 허가를 받고, 나라장터 입찰참가자격에 시설경비업(예: 업종코드 1164)으로 등록한 업체`",
        "- **지역제한**: `입찰공고일 전일부터 입찰일까지 주된 영업소의 소재지가 부산광역시에 있는 업체. 낙찰자는 계약체결일까지 이를 유지하여야 한다.`",
        "- **과업범위**: `청사 출입통제, 순찰, 야간ㆍ휴일 경비, 비상상황 보고, 민원 응대, 경비원 교육 및 근무관리`",
        "- **평가항목**: `현장 책임자 배치계획, 결원 대체인력 투입계획, 비상연락체계, 경찰ㆍ소방 등 유관기관 협력계획, 근로조건 이행계획`",
        "",
        "### 7. 부당제한을 피하는 교정 기준",
        "| 위험한 조건 | 교정 방향 |",
        "|---|---|",
        "| `부산 청사 경비 실적만 인정` | `시설경비업 허가를 받은 업체의 유사 규모 시설경비 수행실적`처럼 과업 규모와 난이도 중심으로 씁니다. |",
...
def _is_security_service_regional_practice_question(user_message: str) -> bool:
    q = (user_message or "").replace(" ", "").lower()
    if not q:
        return False
    has_security_service = (
        ...
        or "무인경비" in q
        or "기계경비" in q
        or ("경비" in q and "용역" in q)
    )
    has_regional_method = any(term in q for term in (
        "부산업체", "지역업체", "부산", "지역제한", "면허", "허가", "발주"
    ))
    has_practice_ask = any(term in q for term in (
        "검토", "방법", "어떻게", "가능", "할수", "되나", "발주", "입찰", "공고"
    ))
    return has_security_service and has_regional_method and has_practice_ask


def _is_public_corp_security_camera_regional_practice_question(user_message: str, agency_type: str | None = None) -> bool:
    q = (user_message or "").replace(" ", "").lower()
    if not q:
        return False
    agency_key = _normalize_agency_type(agency_type) if agency_type else "default"
    has_public_corp_context = (
        agency_key == "public_corporation"
        or any(term in q for term in ("국가공기업", "공기업", "준정부", "공공기관", "공공기관운영법"))
    )
    has_camera_item = any(term in q for term in (
        "보안용카메라", "cctv", "씨씨티비", "영상감시장치", "감시카메라", "폐쇄회로"
    ))
    has_regional_or_local = any(term in q for term in (
        "부산", "지역업체", "부산업체", "지역제한", "우선검토", "우선", "입찰", "구매"
    ))
    has_practice_ask = any(term in q for term in (
        "가능", "할수", "되나", "검토", "구매", "입찰", "발주", "우대", "우선"
    ))
    return has_public_corp_context and has_camera_item and has_regional_or_local and has_practice_ask


def _public_corp_security_camera_regional_lines() -> list[str]:
    return [
        "### 2. 먼저 기관 유형을 확정해야 합니다",
        "- **국가 공기업ㆍ준정부기관**이면 지방계약법이나 부산시 조례의 지역제한 기준을 그대로 가져오지 않습니다.",
        "- 우선순위는 `공공기관운영법상 기관 유형 확인 → 공기업ㆍ준정부기관 계약사무규칙 → 기관 자체 계약규정 → 준용 국가계약법령`입니다.",
        "- 만약 지방공기업이면 별도입니다. 그 경우에는 지방공기업법령과 기관 계약규정에서 지방계약법령을 어느 범위까지 준용하는지 먼저 확인해야 합니다.",
        "",
        "### 3. 보안용카메라(CCTV)는 계약대상부터 나눠야 합니다",
        "| 실제 과업 | 우선 보는 기준 | 부산업체 연결 방식 | 주의할 점 |",
        "|---|---|---|---|",
        "| 카메라ㆍ녹화장치 등 물품 구매 | 세부품명, 중소기업자간 경쟁제품, 직접생산확인, 종합쇼핑몰/MAS 등록 | 부산 소재 조달등록 업체ㆍMAS 공급업체ㆍ직접생산 업체를 후보로 비교 | 부산 소재만으로 특정 업체 지정이나 수의계약 근거가 되지는 않음 |",
        "| 설치ㆍ배선ㆍ네트워크 연동 포함 | 정보통신공사업, 분리발주 또는 분담이행 가능성 | 부산 정보통신공사업체의 현장 대응성, 유지보수 체계를 평가요소로 검토 | 물품 구매에 공사ㆍSI 범위를 묶으면 면허와 책임 범위가 불명확해질 수 있음 |",
        "| 관제 소프트웨어ㆍ영상분석 포함 | 소프트웨어사업자, 보안 요구사항, 기존 시스템 연동 | 지역 유지보수망, 장애 대응 시간, 보안 패치 계획을 평가 | 특정 솔루션이나 특정 업체 규격으로 좁히면 부당제한 리스크가 커짐 |",
        "| 무인경비ㆍ출동 대응까지 포함 | 기계경비업ㆍ시설경비업과 장비 설치 범위 구분 | 경비업 허가 업체와 정보통신 업체의 분담이행을 검토 | 경비용역과 CCTV 설치를 한 업체가 모두 해야 한다고 단정하지 않음 |",
        "",
        "### 4. 지역제한ㆍ지역업체 우선 검토 방식",
        "- 국가 공기업ㆍ준정부기관은 **공기업ㆍ준정부기관 계약사무규칙과 자체 계약규정**에서 지역제한, 제한경쟁, 공동계약, 평가항목을 허용하는지 먼저 봅니다.",
        "- 지역제한을 쓸 수 있는 경우에도 `부산업체를 우대하기 위해`가 아니라 **계약목적, 납품ㆍ설치 장소, 유지보수 대응, 경쟁 가능 업체 수**와 연결해 사유를 남겨야 합니다.",
        "- 종합쇼핑몰/MAS 구매라면 지역제한 입찰과 제도가 다릅니다. 공급업체 소재지, 납품 가능 지역, A/S망, 2단계 경쟁 대상 여부를 따로 확인하세요.",
        "- 특정 부산업체를 바로 지정하기보다, **부산 후보군 조사 → 세부품명ㆍ직접생산ㆍMAS 등록 확인 → 설치ㆍ유지보수 자격 확인 → 평가항목 또는 공동수급 설계** 순서가 안전합니다.",
        "",
        "### 5. 공고ㆍ검토서에 남길 문구 방향",
        "- `부산업체라서 우선 선정`이 아니라 `현장 장애 대응, 납기, 유지보수, 보안사고 긴급출동, 기존 관제시스템 연동 안정성`처럼 계약이행과 연결된 요소로 씁니다.",
        "- 지역업체 후보표는 계약 가능 결론이 아니라 **시장조사 자료**로 붙이고, 최종 판단은 기관 계약규정과 입찰ㆍ구매 방식별 근거로 분리해 적습니다.",
        "- 검토서에는 `지방계약법 기준을 그대로 적용하지 않음`, `공기업ㆍ준정부기관 계약사무규칙 및 자체규정 확인 필요`, `CCTV 물품ㆍ정보통신공사ㆍ경비용역 범위 분리`를 명시하는 편이 안전합니다.",
    ]
...
def _build_practice_manual_fast_answer(user_message: str, agency_type: str | None) -> tuple[str, list[dict]]:
    ...
    is_security_service_regional_question = _is_security_service_regional_practice_question(user_message)
    is_public_corp_security_camera_regional_question = _is_public_corp_security_camera_regional_practice_question(
        user_message,
        agency_key,
    )
    ...
    special_practice_question = any((
        ...
        is_public_corp_security_camera_regional_question,
        is_service_regional_restriction_question,
        is_service_local_participation_question,
        is_general_local_supplier_award_question,
    ...
    if is_lifecycle_procedure_question:
        ...
        if lifecycle_cards:
            is_goods_purchase_flow = (
                contract_object == "goods"
                or "물품" in q
                or any(term in q for term in ("구매절차", "조달절차", "납품", "검수", "대금지급"))
            )
            if is_goods_purchase_flow:
                sections = [
                    "### 1. 질문의도 파악",
                    "- 이 질문은 특정 금액의 가능/불가능 판단이 아니라, 물품 구매를 기본계획부터 검사·검수와 대금지급까지 어떻게 흘려야 하는지 묻는 절차형 질문입니다.",
                    "- 아래 흐름은 표준 워크플로우입니다. 실제 적용 법령은 발주기관 유형에 따라 먼저 확정해야 합니다.",
                    "",
                    "### 2. 먼저 정해야 할 법체계",
                    "| 발주기관 유형 | 우선 확인할 기준 | 실무 의미 |",
                    "|---|---|---|",
                    "| 국가기관 | 국가계약법령, 계약예규, 조달사업법 | 중앙조달·조달요청 의무, 국가 기준의 제한경쟁·수의계약 사유를 먼저 확인 |",
                    "| 지방자치단체 | 지방계약법령, 행안부 예규, 조달사업법 | 지역제한, 지역업체 참여, 지방계약 기준의 수의계약·검사·대가 지급 절차 확인 |",
                    "| 공기업·준정부기관 | 공기업·준정부기관 계약사무규칙, 자체 계약규정, 준용되는 국가계약법령 | 기관 자체 기준과 준용 규정을 먼저 보고, 지방계약 기준을 그대로 가져오지 않음 |",
                    "| 지방공기업·출자출연기관 | 설립 근거 법령, 자체 계약규정, 위탁·보조 조건, 준용 법령 | 기관 유형과 재원 조건에 따라 지방계약법 준용 여부를 별도 확인 |",
                    "",
                    "### 3. 물품 구매 표준 워크플로우",
                    "| 단계 | 핵심 의사결정 | 산출물·데이터 | 법적 허들 |",
                    "|---|---|---|---|",
                    "| 1. 계획 및 사전 행정 | 왜 사는지, 예산으로 가능한지, 직접구매·조달요청 대상인지 결정 | 기본계획, 예산배정 자료, 수요조사, 규격서 초안, 사전검토 체크리스트 | 계약심사·일상감사 대상, 조달요청 의무, 중소기업자간 경쟁제품·직접생산확인 여부 |",
                    "| 2. 가액 산정 및 계약방법 결정 | 추정가격을 산출하고 경쟁입찰·수의계약·종합쇼핑몰/MAS 중 경로 선택 | 시장조사표, 견적서, 거래실례가, 원가계산 자료, 추정가격·기초금액 산출표 | VAT 제외 추정가격과 VAT 포함 기초금액 구분, 수의계약 사유, 지역제한·중소기업 제한 가능 여부 |",
                    "| 3. 공고·견적·낙찰자 결정 | 공급자를 어떻게 선정할지 확정 | 입찰공고문, 참가자격, 평가기준, 제출서류, 질의답변, 개찰·평가 결과 | 특정 브랜드·업체 유리 조건 금지, 적격심사·협상계약·2단계경쟁 기준, 평가위원회·공고기간 준수 |",
                    "| 4. 계약 체결 및 이행관리 | 계약문서와 납품 책임을 확정하고 납기·변경을 관리 | 계약서, 산출내역서, 계약보증, 납품지시서, 변경승인 문서, 납품 진행 기록 | 계약보증금·면제 사유, 납기 연장·규격 변경·물가변동 처리, 분할발주·부당변경 리스크 |",
                    "| 5. 검사·검수 및 대금지급 | 규격대로 납품됐는지 확인하고 예산을 집행 | 검사·검수조서, 납품서, 사진·성능시험 자료, 세금계산서, 청구서, 하자보증 자료 | 검사·검수 기한, 지체상금, 하자담보, 대가 지급기한, 검수 불합격 시 보완·반품 절차 |",
                    "",
                    "### 4. 실무자용 핵심 체크표",
                    "| 구간 | 반드시 확인할 것 | 흔한 실수 |",
                    "|---|---|---|",
                    "| 규격서 | 성능·규격·납품조건을 객관 기준으로 작성 | 특정 브랜드 카탈로그를 그대로 옮기거나 `동등 이상` 기준을 비워둠 |",
                    "| 가격 | 추정가격, 기초금액, 예정가격을 구분 | VAT 포함 예산을 그대로 수의계약·지역제한 기준과 비교 |",
                    "| 품목 자격 | 세부품명, 중소기업자간 경쟁제품, 직접생산확인, 인증·혁신제품 여부 | 부산업체 여부만 보고 법정 품목 자격을 놓침 |",
                    "| 조달 경로 | 종합쇼핑몰/MAS, 제3자단가, 자체 입찰, 2인 이상 견적 중 선택 | 표준 물품인데 자체 수의계약부터 검토하거나, 반대로 모든 물품을 MAS로만 단정 |",
                    "| 정산 | 검사·검수, 지체상금, 하자보증, 대금지급 서류 | 납품 사실만 보고 규격·성능 확인 없이 대금 지급 |",
                    "",
                    "### 5. 지역업체 구매 확대 연결 지점",
                    "- 계획 단계에서 부산 조달등록 업체와 시장 공급 가능성을 조사합니다.",
                    "- 계약방법 결정 단계에서 지역제한, 2인 이상 견적, 종합쇼핑몰/MAS, 기술개발제품·혁신제품, 정책기업 경로를 함께 비교합니다.",
                    "- 공고·평가 단계에서는 `부산업체라서 우대`가 아니라 납기, A/S, 현장 대응, 지역 서비스망, 공동수급 참여 같은 정당한 요소로 설계합니다.",
                    "- 검사·검수 단계에서는 납품 업체와 실제 제조·공급·설치 주체가 일치하는지 확인해야 지역업체 수주 실적도 실질적으로 관리됩니다.",
                ]
            else:
                sections = [
                    "### 1. 질문의도 파악",
    ...
    if is_construction_period_cost_adjustment_question:
        ...
        is_security_camera_question = any(
            term in q
            for term in ("보안용카메라", "cctv", "폐쇄회로", "ip카메라", "영상감시", "관제")
        )
        try:
            from policies.numeric_basis_policy import get_numeric_display
            ...
            from importlib import import_module
            get_numeric_display = import_module("app.policies.numeric_basis_policy").get_numeric_display
        mas_sme_threshold = get_numeric_display("P_MAS_SECOND_STAGE_SME_COMPETITION_THRESHOLD") or "최신 기준 확인 필요"

        if is_security_camera_question:
            sections.extend([
                "",
                "### 2. 보안용카메라(CCTV) 직접생산확인 우선순위",
                "- 보안용카메라·CCTV는 단순 물품 구매로 끝나지 않고 설치, 배선, 녹화장치, 관제 소프트웨어가 결합되는 경우가 많습니다. 따라서 **직접생산확인 → 종합쇼핑몰/MAS 후보 매칭 → 설치·연동 자격 확인** 순서로 봐야 합니다.",
                "",
                "| 순서 | 확인 대상 | 실무 확인 방법 | 판단 의미 |",
                "|---|---|---|---|",
                "| 1 | 세부품명번호 | 폐쇄회로텔레비전시스템, IP카메라 등 실제 구매 세부품명과 규격을 먼저 확정 | 직접생산확인과 MAS 등록 상품을 같은 기준으로 비교할 수 있음 |",
                "| 2 | SMPP 직접생산확인 | 공공구매종합정보망 또는 증명서로 업체명, 사업자번호, 세부품명, 유효기간, 직접생산 범위를 대조 | 세부품명 불일치나 유효기간 만료 시 후보에서 제외 |",
                "| 3 | 종합쇼핑몰/MAS 후보 | 직생 요건을 충족한 업체 중 나라장터 등록 상품, 계약 상태, 납품 가능 지역, 사양 일치를 확인 | 법정 자격을 갖춘 후보만 쇼핑몰 후보군으로 압축 |",
                "| 4 | 설치·연동 범위 | 카메라 설치, 배선, NVR·저장장치, VMS·관제 연동, 유지보수 포함 여부 확인 | 물품 구매인지, 정보통신공사·SW 연동을 포함한 복합 계약인지 결정 |",
                "",
                "### 3. 보안용카메라 특유의 자격 체크",
                "| 체크 항목 | 확인 내용 | 실무 리스크 |",
                "|---|---|---|---|",
                "| 직접생산확인 | 카메라 본체만인지, 폐쇄회로텔레비전시스템 전체인지, NVR·녹화장치까지 포함되는지 확인 | 일부 구성품만 직생인 업체를 전체 시스템 후보로 보면 계약·검수 단계에서 문제가 생길 수 있음 |",
                "| 정보통신공사업 | 카메라 설치, 케이블 포설, 네트워크 장비 연결 등 현장 시공 범위 확인 | 물품 단가에 설치가 포함되어도 실제 시공은 관련 면허와 책임 범위를 별도로 봐야 함 |",
                "| 소프트웨어사업자 | VMS, 관제 프로그램, 영상분석, 기존 시스템 연동이 포함되는지 확인 | SI 성격이 강한데 물품 구매 자격만 두면 과업 수행 능력 검증이 부족해짐 |",
                "| 유지보수·A/S | 장애 대응 시간, 부산 현장 출동 가능성, 부품 공급 기간, 보안 패치 계획 확인 | 지역업체의 장점은 단순 소재지가 아니라 빠른 현장 대응성과 유지보수 체계로 평가해야 안전함 |",
                "",
                "### 4. 부산업체와 종합쇼핑몰 후보를 연결하는 방식",
                f"- 중소기업자간 경쟁제품의 MAS 2단계 경쟁 기준은 현재 내부 기준상 **{mas_sme_threshold} 이상**으로 관리되지만, 기준은 바뀔 수 있으므로 공고 전 최신 조달청 기준을 확인해야 합니다.",
                "- 기준 미만이면 종합쇼핑몰에서 직접생산확인과 세부품명이 맞는 부산 소재 제조사·공급사를 우선 후보로 비교할 수 있습니다.",
                "- 기준 이상이면 2단계 경쟁에서 복수 업체 제안요청을 전제로 하되, 부산업체 자체를 무조건 찍기보다 **현장 유지보수 대응성, 납기, 장애 복구 계획, 지역 서비스망** 같은 정당한 평가요소로 부산업체의 강점을 반영합니다.",
                "- 부산 대리점과 부산 제조사는 구분해서 보세요. 지역경제 효과나 정책 성과를 중시한다면 본점·공장·직접생산 소재지까지 확인하는 편이 좋습니다.",
            ])
        else:
            sections.extend([
                "",
                ...
                "| 순서 | 확인 대상 | 실무 확인 방법 | 판단 의미 |",
                "|---|---|---|---|",
                "| 1 | 세부품명 확정 | 세부품명번호, 규격, 구성품, 설치 포함 여부를 먼저 확정 | 직접생산확인과 쇼핑몰 상품을 같은 기준으로 비교 |",
                "| 2 | 직접생산확인 | SMPP 또는 증명서로 세부품명, 업체명, 유효기간, 직접생산 범위 대조 | 자격 없는 업체를 후보에서 제외 |",
                "| 3 | 종합쇼핑몰/MAS | 조달등록 상품, 계약 상태, 납품 가능 지역, 사양 일치 여부 확인 | 실제 구매 가능한 후보군으로 압축 |",
                "| 4 | 지역업체 활용 | 부산 소재 여부, 직접생산 소재지, A/S·납기 대응성 확인 | 지역업체 수주 지원을 법정 자격과 연결 |",
                f"- MAS 2단계 경쟁 기준은 현재 내부 기준상 중소기업자간 경쟁제품 **{mas_sme_threshold} 이상**으로 관리되지만, 기준은 바뀔 수 있으므로 최신 조달청 기준을 공고 전 다시 확인해야 합니다.",
            ])
        ...
        sections.extend([
            "",
            "### 2. 결론: `주된 계약 목적`과 `원가·산출물 비중`으로 판단합니다",
            "- 홍보물 디자인, 편집, 인쇄, 납품이 한 사업에 섞이면 단순히 `디자인이 있으니 용역`, `인쇄물이 있으니 물품`으로 정하면 안 됩니다.",
            "- 먼저 발주기관이 사려는 핵심 가치가 **창작·기획·편집이라는 지적 노무**인지, 아니면 **규격화된 인쇄물의 제작·납품**인지 판단합니다.",
            "- 원가 구성도 중요한 판단자료입니다. 다만 `50% 초과` 같은 비중은 실무상 참고선으로 쓰되, 법령상 자동 분류 기준처럼 단정하지 말고 과업의 본질, 산출물, 자격요건, 검수방식을 함께 봐야 합니다.",
            "",
            "### 3. 용역과 물품 제조 판단표",
            "| 구분 | 용역 성격이 강한 경우 | 물품 제조·구매 성격이 강한 경우 |",
            "|---|---|---|",
            "| 핵심 가치 | 기획, 카피, 디자인 콘셉트, 편집, 저작권 처리 | 종이, 잉크, 후가공, 대량 생산, 정해진 수량 납품 |",
            "| 원가 구성 | 디자인·편집 인건비, 기획비, 저작권 비용 비중이 큼 | 인쇄비, 용지비, 제본·후가공비, 배송비 비중이 큼 |",
            "| 주요 산출물 | 원본 파일, 편집 데이터, 디자인 시안, 저작권 양도·이용허락 | 책자, 리플릿, 포스터, 봉투 등 물리적 인쇄물 |",
            "| 검수 기준 | 시안 승인, 교정 횟수, 창작물 품질, 원본파일 제공 여부 | 수량, 규격, 색상, 지질, 제본, 포장, 납품기한 |",
            "| 적합한 계약 방식 | 협상에 의한 계약, 제안서 평가가 필요한 용역 | 물품 제조·구매, 규격·단가·납품 중심 경쟁 |",
            "",
            "### 4. 자격요건 설계 차이",
            "| 분류 | 참가자격·확인서류 | 주의점 |",
            "|---|---|---|",
            "| 용역 중심 | 산업디자인전문회사, 시각디자인·편집디자인 수행실적, 제안서 평가, 저작권 처리 능력 | 인쇄 설비 보유를 과도하게 요구하면 디자인 전문업체를 부당하게 배제할 수 있습니다. 인쇄는 협력 또는 하도급 가능 범위를 과업서에 명확히 둡니다. |",
            "| 물품 제조 중심 | 인쇄사 신고, 인쇄물 관련 직접생산확인증명서, 세부품명·규격 일치, 납품·검수 능력 | 디자인 면허만 있는 업체가 낙찰 뒤 실제 인쇄를 재하청하는 구조가 되지 않도록 직접생산·제조능력 확인이 필요합니다. |",
            "| 혼합 일괄 발주 | 주된 계약목적에 맞춘 자격을 기본으로 하고, 부수 업무는 협력·분담이행·하도급 관리 조건으로 정리 | 용역 자격과 물품 제조 자격을 모두 필수로 걸면 경쟁을 과도하게 줄일 수 있으므로 필수요건과 평가요소를 분리합니다. |",
            "",
            "### 5. 실무 판단 순서",
            "1. **예산 산출내역 분리**: 디자인·편집비, 인쇄비, 용지·후가공비, 배송비를 나눠 어느 부문이 사업의 중심인지 봅니다.",
            "2. **최종 산출물 확인**: 기관이 실제로 필요한 것이 창작 원본과 활용권인지, 정해진 부수의 인쇄물 납품인지 확인합니다.",
            "3. **검수 기준 확인**: 디자인 승인·저작권 검수가 중심이면 용역, 수량·규격·납품검수가 중심이면 물품 제조 쪽에 가깝습니다.",
            "4. **자격요건 검토**: 산업디자인전문회사, 인쇄사 신고, 직접생산확인 중 어느 요건이 계약 이행에 필수인지 구분합니다.",
            "5. **분리발주 가능성 검토**: 디자인 개발과 대량 인쇄가 독립적으로 수행 가능하면 정당한 분리발주도 검토할 수 있습니다. 다만 금액 기준 회피처럼 보이는 쪼개기 발주는 피해야 합니다.",
            "",
            "### 6. 부산업체 수주지원 설계",
            "| 분류 결과 | 부산업체 지원 방향 | 공고·평가 설계 포인트 |",
            "|---|---|---|",
            "| 용역 중심 | 부산 디자인·기획사의 참여 가능성을 높임 | 지역 이해도, 현장 커뮤니케이션, 지역 콘텐츠 활용계획, 사후 수정 대응을 협상계약 평가항목에 반영 |",
            "| 물품 제조 중심 | 부산 인쇄업체·직접생산 업체의 납품 가능성을 높임 | 직접생산확인, 납기, 품질관리, 색상교정, 포장·배송, 하자 대응을 객관 요건으로 설계 |",
            "| 대형·복합 사업 | 부산업체 공동수급·분담이행 유도 | 디자인과 인쇄 역할을 나눠 공동수급 또는 협력체계를 허용하고, 실제 수행분담·책임범위를 평가 |",
            "",
            "### 7. 판단 요약",
            "- 창의적 기획·디자인·저작권 확보가 중심이면 **용역**으로 보는 것이 자연스럽습니다.",
            "- 정해진 규격의 홍보물을 대량 제작해 납품받는 것이 중심이면 **물품 제조·구매** 성격이 강합니다.",
            "- 대부분의 단순 리플릿·책자 제작은 인쇄물 납품 비중이 커서 물품 제조 성격을 함께 검토해야 하지만, 도시브랜딩·캐릭터 개발·캠페인 콘셉트 개발처럼 무형 창작물이 핵심이면 용역으로 분류하는 편이 안전합니다.",
            "- 부산업체 지원은 특정 업체 지정이 아니라, 분류 결과에 맞춰 지역제한 가능성, 공동수급, 협상계약 평가항목, 직접생산확인, 납기·품질관리 기준으로 설계해야 합니다.",
        ])
        ...
        sections.extend(["", *_bid_notice_local_company_lines()])
    ...
    elif is_public_corp_security_camera_regional_question:
        sections.extend(["", *_public_corp_security_camera_regional_lines()])
    elif is_security_service_regional_question:
        sections.extend(["", *_security_service_regional_lines()])
        ...
        sections.extend([
            "",
            "### 2. 결론: 법령 한도는 보통 `추정가격`, 낙찰 판단은 `예정가격`을 봅니다",
            "- 공공계약의 가격 용어는 단순 회계용어가 아니라 **계약방법, 지역제한, 수의계약 가능성, 낙찰자 결정**을 가르는 기준입니다.",
            "- 가장 흔한 실수는 부가가치세가 포함된 총예산을 법령상 금액 기준과 바로 비교하는 것입니다.",
            "- 수의계약, 지역제한, 국제입찰 대상 여부 같은 한도 판단은 대체로 **추정가격(VAT 제외)**을 기준으로 보되, 조문·기관유형·계약대상별 예외는 최신 법령·고시로 확인해야 합니다.",
            "",
            "### 3. 가격 용어 실무 비교표",
            "| 용어 | VAT 처리 | 주로 쓰는 시점 | 실무 용도 | 주의할 점 |",
            "|---|---|---|---|---|",
            "| 추정가격 | 보통 VAT 제외 | 사업계획·계약방법 결정 단계 | 수의계약, 2인 이상 견적, 지역제한, 국제입찰, 공고기간 등 법령상 금액 기준 판단 | 법령 한도와 대조할 때 가장 먼저 확인합니다. VAT 포함 총액을 그대로 대조하면 오판할 수 있습니다. |",
            "| 기초금액 | 보통 VAT 포함 | 입찰공고·예정가격 작성 전 | 예정가격 산정을 위한 공개 기준금액, 입찰자가 투찰률을 계산할 때 보는 기준 | 시장조사, 거래실례가격, 원가계산 근거를 남겨야 합니다. 너무 낮으면 유찰, 너무 높으면 예산낭비 리스크가 있습니다. |",
            "| 예정가격 | 보통 VAT 포함 | 개찰·낙찰자 결정 단계 | 낙찰 가능 상한선, 적격심사·낙찰률 산정 기준 | 예정가격을 초과한 입찰은 원칙적으로 낙찰대상이 되기 어렵습니다. 복수예비가격 방식 등 기관 절차를 확인합니다. |",
            "| 추정금액 | 공사에서 VAT·관급자재 등을 포함해 규모 판단에 사용되는 경우가 많음 | 공사 규모·실적제한·시공능력 검토 단계 | 사업 전체 규모, 공사 실적제한, 시공능력평가액 비교 등에 활용 | 추정가격과 혼동하면 참가자격·실적제한 기준을 잘못 잡을 수 있습니다. 관급자재 포함 범위를 공고문에 명확히 해야 합니다. |",
            "",
            "### 4. 가격 산정의 흐름",
            "| 단계 | 실무자가 하는 일 | 중심 금액 | 판단 포인트 |",
            "|---|---|---|---|",
            "| 1. 계획·예산 검토 | 사업비, 물량, 시장가격, 원가계산 자료를 모아 계약방법을 정함 | 추정가격 | VAT 제외 기준으로 수의계약·지역제한·입찰방법 가능성을 먼저 판단합니다. |",
            "| 2. 공고 준비 | 거래실례가격, 견적, 원가계산을 바탕으로 입찰 기준가를 정리함 | 기초금액 | 입찰자에게 공개되는 기준이므로 산정근거와 조사자료를 남깁니다. |",
            "| 3. 개찰·낙찰 판단 | 복수예비가격 추첨·산술평균 등 절차로 최종 기준가격을 확정함 | 예정가격 | 투찰금액이 예정가격 이하인지, 낙찰하한율·적격심사 기준을 충족하는지 봅니다. |",
            "| 4. 공사 규모·자격 확인 | 관급자재, 도급 범위, 실적제한, 시공능력 등을 확인함 | 추정금액 | 공사에서 사업 규모와 업체 수행능력을 볼 때 추정금액을 별도로 확인합니다. |",
            "",
            "### 5. VAT 실수 방지 체크포인트",
            "- 법령이나 예규에서 `추정가격 ○○원 이하`라고 되어 있으면 먼저 **VAT 제외 금액**인지 확인합니다.",
            "- 예를 들어 어떤 기준이 `추정가격 2,000만원 이하`라면, 산술적으로 VAT 포함 총액은 2,200만원 수준이 될 수 있습니다. 다만 이 숫자는 설명용 예시이고, 실제 기준금액은 최신 법령·고시와 계약대상별 규정을 확인해야 합니다.",
            "- 원가계산서나 내부 검토서에는 `추정가격 / 부가가치세 / 관급자재 / 추정금액 / 기초금액 / 예정가격`을 칸으로 나눠 쓰면 공고문 작성 때 혼선을 줄일 수 있습니다.",
            "- 특히 지역제한, 수의계약, 정책기업 1인 견적, MAS 2단계 경쟁 기준을 볼 때는 `총예산`이 아니라 해당 제도가 요구하는 기준금액이 무엇인지 먼저 확인해야 합니다.",
            "",
            "### 6. 한 줄 정리",
            "- **계약방법을 정할 때는 추정가격**, **입찰자가 얼마까지 써야 하는지 볼 때는 예정가격**, **공고 기준가를 볼 때는 기초금액**, **공사 전체 규모와 자격 제한을 볼 때는 추정금액**을 확인하세요.",
        ])
...
def _build_grounded_case_timeout_fallback(user_message: str, mcp_context: str = "") -> str:
    ...
    if "사회적협동조합" in q and "수의계약" in q and any(term in q for term in ("5000만원", "5천만원", "오천만원", "한도", "가능")):
        ...
        return "\n".join([
            ...
            f"- 결론부터 보면, **요건을 갖춘 사회적협동조합**이고 질문 금액이 **VAT 제외 추정가격 {policy_label} 이하**라면 1인 견적 수의계약 가능성을 검토할 수 있습니다.",
            f"- 질문 금액 기준 판단은 **{judgment}**입니다. 다만 **사회적협동조합 인가만으로는 부족**하고, 취약계층 고용비율과 확인서 등 특례 요건을 먼저 검증해야 합니다.",
            "",
            "### 법적 근거",
            "- **수의계약 사유**: 「지방계약법 시행령」 제25조 제1항 제5호 가목의 사회적기업ㆍ사회적협동조합 등 정책기업 관련 수의계약 사유를 확인합니다. 내부 검토서에는 사회적협동조합 해당 목과 적용 사유를 최신 법령 원문으로 대조해 적으세요.",
            f"- **1인 견적 방식**: 「지방계약법 시행령」 제30조 제1항 제2호에 따라 사회적협동조합 등은 정해진 요건을 충족하는 경우 **추정가격 {policy_label} 이하**에서 1인 견적 방식을 검토할 수 있습니다.",
            "- **핵심 요건**: 사회적협동조합 중에서도 **취약계층 고용비율 30% 이상** 등 행정안전부장관이 정하는 요건을 갖춘 경우인지 확인해야 합니다.",
            "",
            "### 필수 요건",
            "1. **사회적협동조합 지위**: 협동조합 기본법상 사회적협동조합 인가증, 사업자등록, 정관ㆍ사업 범위를 확인합니다.",
            "2. **취약계층 고용비율**: 전체 근로자 중 취약계층 고용비율이 30% 이상인지 확인합니다. 이 요건이 확인되지 않으면 5천만원 1인 견적 특례를 바로 적용하면 안 됩니다.",
            "3. **수의계약 대상 확인서**: 수의계약 체결 대상 사회적협동조합 확인서 또는 이에 준하는 확인자료를 발급ㆍ확인기관을 통해 확인하고, 유효기간과 적용 대상 사업을 품의서에 붙입니다.",
            "4. **직접생산ㆍ직접공급**: 해당 제품을 실제로 직접 생산ㆍ공급할 수 있는지 확인합니다. 중소기업자간 경쟁제품이면 직접생산확인증명서와 세부품명 일치를 확인해야 합니다.",
            "5. **가격 적정성**: 1인 견적이라도 시장가격, 조달가격, 거래실례가격, 타사 비교견적 등을 근거로 가격이 적정한지 남깁니다.",
            "",
            "### 실무 프로세스",
            "1. **추정가격 확정**: VAT 제외 추정가격이 5천만원 이하인지 확인합니다. VAT 포함 총액ㆍ예정가격ㆍ계약금액과 혼동하지 마세요.",
            "2. **자격검증**: 사회적협동조합 인가증, 취약계층 고용비율 확인자료, 수의계약 대상 확인서, 수의계약 체결 제한 여부 확인서, 지방세ㆍ국세 납세증명을 확인합니다.",
            "3. **품목검증**: 조달등록, 직접생산확인증명서, 제품 규격 적합성, 납품ㆍA/S 가능성, 실제 공급 주체를 확인합니다.",
            "4. **내부 한도관리**: 동일 업체와의 연간 수의계약 횟수ㆍ총액, 부산시 또는 기관 내부 수의계약 총량제ㆍ반복계약 제한 기준을 확인합니다.",
            "5. **계약문서 작성**: 견적서, 수의계약 사유서, 청렴서약서, 제한 여부 확인서, 가격조사표, 계약정보 공개자료를 준비합니다.",
            "",
            "### 감사 대응 포인트",
            "- `사회적협동조합이라서 가능`이 아니라 **취약계층 고용비율 30% 이상 등 요건을 갖춘 사회적협동조합이고, 추정가격 5천만원 이하이며, 해당 제품을 직접 공급할 수 있다**는 구조로 쓰세요.",
            "- 부산 소재 조합이면 지역상품 구매 실적과 사회적 가치 실적을 함께 관리할 수 있지만, 부산 소재 자체가 독립적인 수의계약 사유는 아닙니다.",
            "- 5천만원에 근접한 계약은 분할발주 의심을 받기 쉬우므로, 당해연도 동일ㆍ유사 제품 구매계획과 수요 취합 근거를 먼저 확인하세요.",
        ])
    if (
        ...
        return "\n".join([
            ...
            f"- 결론부터 보면, 여성기업 1인 견적 기준은 **VAT 제외 추정가격 {one_quote_policy} 이하**가 핵심이고, 질문 금액은 **{judgment}**입니다.",
            "- 다만 소프트웨어는 바로 수의계약 사유서로 들어가기보다 **SW 사전 행정절차와 조달청 디지털서비스몰·종합쇼핑몰 등록 여부**를 먼저 확인하는 것이 안전합니다.",
            "",
            "### 법적 근거",
            "- **수의계약 사유**: 「지방계약법 시행령」 제25조 제1항 제5호의 여성기업 등 정책기업 관련 수의계약 사유를 확인합니다. 세부 목ㆍ호 표기는 최신 법령 원문으로 최종 대조하세요.",
            f"- **1인 견적 방식**: 「지방계약법 시행령」 제30조 제1항 제2호 단서에 따라 여성기업 등 정책기업과 계약하는 경우 **추정가격 {one_quote_policy} 이하**에서 1인 견적 방식을 검토할 수 있습니다.",
            "- **여성기업 확인**: 「여성기업지원에 관한 법률」 제2조 제1호의 여성기업 해당 여부와 SMPP 여성기업 확인서 유효기간을 확인합니다.",
            "",
            "### 진행 순서",
            "1. **SW 사전 행정절차 확인**: 소프트웨어사업 영향평가 대상인지 확인하고, 「소프트웨어 진흥법」 제43조 및 관련 고시에 따른 영향평가 결과서를 남깁니다. 과업내용 확정 심의도 확인하되, 상용SW 구매나 1억원 이하 사업은 간소화 심의 가능성을 함께 봅니다.",
            "2. **보안ㆍ기술 검토**: 보안성 검토, 개인정보 처리 여부, 기존 시스템 호환성, 라이선스 형태(영구 라이선스인지 구독형/SaaS인지), 유지보수 범위를 먼저 정리합니다.",
            "3. **디지털서비스몰ㆍ종합쇼핑몰 검색**: 상용SW 등록 여부, 제3자단가계약, 공급업체, 납품 가능 지역, 기술지원 조건을 확인합니다. 등록 상품이면 쇼핑몰 구매 또는 납품요구 경로를 우선 검토합니다.",
            "4. **여성기업 1인 견적 요건 확인**: SMPP 여성기업 확인서, 사업자등록, 소프트웨어사업자 신고확인서, 직접생산확인증명서(개발ㆍ구축 포함 또는 대상 품목인 경우)를 확인합니다.",
            "5. **가격 적정성 확보**: 1인 견적이라도 거래실례가격, 쇼핑몰 가격, 유사 SW 견적, 유지보수 요율 등을 비교해 산출기초를 남깁니다.",
            "6. **계약 문서 작성**: 견적서, 수의계약 사유서, 청렴서약서, 보안서약서, 기술지원확약서ㆍ공급증명원 필요 여부, 과업지시서 또는 규격서를 정리합니다. 수의계약 배제ㆍ제한 대상 여부와 계약정보 공개 절차도 확인하세요.",
            "",
            "### 실무 체크리스트",
            f"- **금액 한도**: {amount_label}이 VAT 제외 추정가격인지 확인합니다. 5천만원을 넘으면 여성기업 1인 견적 경로가 아니라 경쟁입찰ㆍ2인 이상 견적 등 다른 경로를 검토해야 합니다.",
            "- **지역 요건**: 부산 소재 여성기업이라는 점은 지역상품 구매지원과 내부 품의의 정책 근거로 연결할 수 있지만, 독립적인 수의계약 사유는 여성기업 지위와 금액 기준입니다.",
            "- **SW 사업 범위**: 단순 상용SW 라이선스 구매인지, 커스터마이징ㆍ개발ㆍ구축ㆍ유지관리 용역이 포함되는지 구분합니다. 개발이 포함되면 과업심의, 직접생산, SW사업자 신고, 하도급 제한을 더 엄격히 봐야 합니다.",
            "- **규격서 리스크**: 라이선스, 유지보수, 보안ㆍ호환성, 기술지원 범위가 특정 제조사 맞춤 조건이 되지 않도록 대체 가능성과 객관 기준을 같이 적습니다.",
            "",
            "### 품의서 문장",
            f"- `본 건은 VAT 제외 추정가격 {amount_label} 소프트웨어 구매로서, 디지털서비스몰ㆍ종합쇼핑몰 등록 여부와 SW 사전 행정절차를 우선 확인하고, 해당 경로가 부적합하거나 미등록인 경우 여성기업 확인서와 지방계약법 시행령 제25조ㆍ제30조의 견적 기준을 근거로 1인 견적 수의계약을 검토한다`고 정리하세요.",
        ])
    if (
        ...
        if over_policy:
            return "\n".join([
                ...
                f"- 결론부터 보면, **{company_type}이라도 1인 견적 수의계약은 VAT 제외 추정가격 {one_quote_policy} 이하인지 먼저 봐야 합니다.**",
                f"- 질문의 {amount_label}이 추정가격이라면 정책기업 1인 견적 한도를 넘으므로, 특정 업체 1곳을 바로 지정하는 방식으로 처리하기 어렵습니다.",
                "",
                "### 법적 근거",
                "- **수의계약 사유**: 「지방계약법 시행령」 제25조 제1항 제5호의 여성기업ㆍ장애인기업ㆍ사회적기업 등 정책기업 관련 수의계약 사유를 확인합니다. 세부 목ㆍ호 표기는 최신 법령 원문으로 최종 대조하세요.",
                f"- **1인 견적 방식**: 「지방계약법 시행령」 제30조 제1항 제2호 단서에 따라 정책기업과 계약하는 경우에도 1인 견적은 **추정가격 {one_quote_policy} 이하**인지가 핵심입니다.",
                f"- **이 건 판단**: {amount_label}이 VAT 제외 추정가격이면 한도를 초과하므로, {company_type}이라는 이유만으로 1인 수의계약을 진행하기는 어렵습니다.",
                "",
                "### 먼저 확인할 금액 기준",
                f"1. **추정가격인지 확인**: {amount_label}이 부가가치세 제외 추정가격인지, VAT 포함 총액ㆍ예정가격ㆍ계약금액인지 구분하세요. VAT 포함 총액이라면 공급가액 기준으로 다시 산정해야 합니다.",
                f"2. **5천만원 이하 여부**: VAT 제외 추정가격이 {one_quote_policy} 이하로 내려가는 경우에만 정책기업 1인 견적 가능성을 다시 검토할 수 있습니다.",
                "3. **쪼개기 금지**: 7천만원 용역을 5천만원 이하로 나누어 같은 목적의 계약을 분할하면 「지방계약법 시행령」 제77조 제1항의 분할계약 금지 취지상 감사 지적 위험이 큽니다.",
                "",
                "### 대안 프로세스",
                f"1. **G2B 2인 이상 견적 소액수의**: 용역 소액수의는 {two_quote} 이하 구간에서 2인 이상 견적 제출 공고를 우선 검토합니다. 지정정보장치(G2B)를 이용하고, 공고문에 제한사항과 기준을 명확히 적습니다.",
                "2. **부산 지역제한 검토**: 지방계약법 제9조 및 시행령ㆍ시행규칙의 제한입찰 체계 안에서 부산광역시 소재 업체로 제한할 수 있는지 확인합니다. 지역제한은 계약목적ㆍ금액ㆍ경쟁가능 업체 수와 함께 검토해야 합니다.",
                f"3. **{company_type} 요건 반영**: 장애인기업 확인서 등 정책기업 확인서 유효기간, 과업 수행능력, 직접수행 여부, 필요한 경우 직접생산확인증명서를 확인합니다. 단, 정책기업이라는 사실만으로 7천만원 1인 지정이 열리지는 않습니다.",
                "4. **협상ㆍ적격심사 활용**: 품질 평가가 중요한 용역이면 협상계약 또는 적격심사를 검토하고, 지역 현장 대응성ㆍ장애인기업 지원 취지는 과업 관련성이 있을 때 평가항목으로 반영합니다.",
                "5. **수의계약 배제사유 확인**: 부정당업자 제재, 수의계약 배제 대상, 계약이행 지연ㆍ품질 문제, 지방계약법 시행령 제92조 관련 제재 이력 등을 확인합니다.",
                "",
                "### 예외적으로 1인 수의가 필요한 경우",
                "- 특정 업체만 수행 가능한 특허ㆍ독점 기술, 저작권, 기존 시스템과의 대체불가능한 연속성 등 별도 수의계약 사유가 있으면 「지방계약법 시행령」 제25조의 다른 사유를 별도로 검토할 수 있습니다.",
                "- 이 경우에도 `부산 소재 장애인기업 지원`은 보조 사유일 뿐이고, 대체곤란성ㆍ기술 독점성ㆍ가격 적정성을 강하게 입증해야 합니다.",
                "",
                "### 정리",
                f"- 이 건은 **{company_type} 1인 수의계약이 아니라 부산 지역제한 2인 이상 견적 공고**를 우선 검토하는 것이 안전합니다.",
                "- 수의계약 사유서에는 `부산업체 지원`만 쓰지 말고, 적용 가능한 법정 사유와 견적 절차, 추정가격 산정근거를 분리해서 남겨야 합니다.",
            ])
    if (
        ...
        if amount is not None:
            ...
            if isinstance(one_quote_general_value, (int, float)) and amount <= one_quote_general_value:
                ...
                return "\n".join([
                    ...
                    f"- 결론부터 보면, 질문 금액이 **부가가치세를 제외한 추정가격 {one_quote_general} 이하**라면 일반 물품도 **1인 견적 수의계약 검토 범위**에 들어갑니다.",
                    "- 부산 소재 업체와 계약을 검토할 수는 있지만, 근거는 `부산업체라서`가 아니라 **소액 수의계약 금액 기준과 견적 방식 요건**입니다.",
                    "",
                    "### 법적 근거",
                    f"1. **수의계약 사유**: 「지방계약법 시행령」 제25조 제1항 제5호 가목의 소액 물품 제조ㆍ구매 계약 기준을 확인합니다. 이 답변은 일반 물품 기준 **추정가격 {one_quote_general} 이하** 구간을 전제로 합니다.",
                    "2. **1인 견적 방식**: 「지방계약법 시행령」 제30조 제1항의 견적 제출 기준을 함께 봅니다. 수의계약 사유가 있더라도 견적 방식은 별도 판단 대상입니다.",
                    "3. **지역업체 우선 검토**: 부산 지역상품 구매 확대 지침이나 조례 취지는 후보 선정ㆍ시장조사ㆍ내부 품의의 정책 근거로 연결할 수 있습니다. 다만 그 자체가 독립적인 수의계약 사유는 아닙니다.",
                    "",
                    "### 상세 검토",
                    f"1. **금액 기준**: {amount_label}이 VAT 제외 추정가격이면 일반 1인 견적 기준인 {one_quote_general} 이하입니다. 예산 총액이나 예정가격이 아니라 추정가격 기준인지 다시 확인하세요.",
                    f"2. **정책기업 특례와 구분**: 여성기업ㆍ장애인기업ㆍ사회적기업 등 정책기업은 별도 1인 견적 기준인 {one_quote_policy} 이하 구간을 검토할 수 있습니다. 이 사안은 일반 소액 기준 안에 있으므로 정책기업 특례와 혼동하지 않아도 됩니다.",
                    f"3. **2인 이상 견적 대안**: 더 투명하게 경쟁 기록을 남기려면 {two_quote} 이하 소액수의 2인 이상 견적도 대안입니다. 특정 업체 지정 필요성이 약하면 이 경로가 감사 대응에 유리할 수 있습니다.",
                    "4. **부산업체 활용**: 부산 소재 여부, 조달등록, 납품 가능 품목, 가격 적정성, 하자ㆍA/S 대응 가능성을 확인하고 시장조사 자료로 남기세요.",
                    "",
                    "### 실무 체크리스트",
                    "- **추정가격/예정가격 구분**: 법령상 한도 비교는 원칙적으로 VAT 제외 추정가격 기준으로 정리하고, 예정가격ㆍ기초금액ㆍ계약금액과 혼동하지 않게 품의서에 분리 기재하세요.",
                    "- **분할발주 금지**: 동일ㆍ유사 물품을 금액 기준에 맞추려고 나누면 「지방계약법 시행령」 제77조 제1항의 분할계약 금지 취지상 지적 위험이 있으므로 수요 취합 근거를 남기세요.",
                    "- **직접생산확인**: 중소기업자간 경쟁제품이면 직접생산확인증명서, 세부품명, 유효기간, 실제 납품 주체를 확인하세요.",
                    "- **내부 품의 문구**: `추정가격 2천만원 이하 1인 견적 검토`, `부산 지역업체 우선 검토 사유`, `가격 적정성 및 공급능력 확인`을 서로 다른 항목으로 적는 것이 안전합니다.",
                ])
    if any(term in q for term in ("분할발주", "쪼개기", "나누어", "나눠", "세번", "3번")) and any(term in q for term in ("동일품목", "동일한품목", "같은품목", "동일수요")):
        ...
        split_count = 3 if any(term in q for term in ("세번", "3번", "3회")) else None
        if amount and split_count:
            total_amount = amount * split_count
            total_label = f"{total_amount // 10_000:,}만원({total_amount:,}원)"
        else:
            total_label = "전체 수요 합산액"
        return "\n".join([
            "### 동일 품목을 나누어 수의계약하는 경우",
            f"- 결론부터 보면, **동일 품목을 {amount_label}씩 여러 번 나누어 부산 업체들과 각각 1인 수의계약**하는 방식은 원칙적으로 피해야 합니다.",
            f"- 질문처럼 세 번 구매라면 실무상 먼저 **총수요 {total_label}**를 기준으로 계약방법을 판단해야 합니다. 전체 수요가 이미 예측 가능했는데 1인 견적 한도에 맞추려고 나누면 `분할발주` 또는 `쪼개기 수의계약`으로 감사 지적을 받을 수 있습니다.",
            "- 부산 업체를 여러 곳 지원하려는 목적이 있어도, 금액 한도 회피처럼 보이면 정당한 지역업체 지원 논리가 아니라 부당한 계약방법 선택으로 평가될 수 있습니다.",
            "",
            "### 법적·실무 근거",
            "1. 「지방계약법 시행령」 제77조는 공사 분할계약 금지 규정이지만, 지방계약 실무에서는 같은 목적의 수요를 수의계약 한도에 맞추기 위해 시기나 물량을 나누는 행위를 분할계약 금지 취지로 엄격하게 봅니다.",
            "2. 물품·용역도 「지방자치단체 입찰 및 계약 집행기준」의 수의계약 운영요령과 감사 기준에서 `동일·유사 물품의 인위적 분할`, `1인 견적 한도 회피`, `동일 업체 또는 관련 업체 반복 계약`을 주요 점검 대상으로 봅니다.",
            "3. 문제가 되면 계약담당자의 주의·경고·징계 요구, 수의계약 사유 부적정 지적, 향후 수의계약 체결 제한 또는 내부 총량제 점검으로 이어질 수 있습니다.",
            "",
            "### 분할발주 판단 기준",
            "1. **예산의 동일성**: 같은 세부사업, 같은 목·세목, 같은 사업계획에서 집행되는 수요인지 확인합니다.",
            "2. **품목의 동일·유사성**: G2B 세부품명·물품분류번호, 규격, 사용 목적, 납품 후 사용처가 같은지 봅니다.",
            "3. **시기적 근접성**: 같은 회계연도 안에서 짧은 기간에 반복 구매하거나, 연간 구매계획에 이미 포함된 수요인지 확인합니다.",
            "4. **금액 기준 회피성**: 각 계약을 2천만원 이하로 맞춘 이유가 납품·규격·예산 재원 차이인지, 아니면 1인 견적 기준 회피인지 봅니다.",
            "",
            "### 안전한 처리",
            f"1. **통합 수요 산정**: 동일 품목의 연간 또는 사업 단위 수요를 먼저 합산해 추정가격을 산정합니다. 이 사안은 우선 {total_label} 통합 발주 가능성을 검토하는 것이 안전합니다.",
            "2. **G2B 2인 이상 견적**: 통합 추정가격이 일반 1인 견적 기준을 넘으면 나라장터 2인 이상 견적 제출 공고를 우선 검토합니다.",
            "3. **부산 지역제한**: 금액 기준과 경쟁 가능한 업체 수가 맞으면 `부산광역시 소재 업체` 지역제한으로 부산 업체 간 경쟁을 설계할 수 있습니다.",
            "4. **조달 경로 검토**: 종합쇼핑몰/MAS, 단가계약, 희망수량 또는 분할납품 조건 등 정상 조달 경로가 있는지 먼저 봅니다.",
            "5. **정당한 분리 사유 문서화**: 납품 시기·장소·규격·예산 재원·기술 요건이 실질적으로 달라 분리 발주가 불가피하다면, 통합 발주 검토 결과와 분리 사유를 결재문서에 남겨야 합니다.",
            "",
            "정리하면, 부산 업체 3곳에 기회를 주려는 취지는 지역제한·견적공고·평가항목으로 풀어야 하고, **동일 품목을 2천만원씩 쪼개 1인 수의계약으로 배분하는 방식은 감사 대응상 매우 취약합니다.**",
        ])
    if any(term in q for term in ("긴급재난", "재난복구", "재난", "긴급복구")) and "공사" in q and "수의계약" in q:
        ...
        return "\n".join([
            ...
            "- 결론부터 보면, **입찰에 부칠 여유가 없는 긴급 재난복구라면 금액만으로 배제하지 않고 재난 수의계약 특례와 1인 견적을 검토할 수 있습니다.**",
            "- 현행 「지방계약법 시행령」은 긴급복구가 필요한 재난복구 등을 제25조 제1항 제2호의 수의계약 사유로 두고, 제30조 제1항 제1호에서 제25조 제1항 각 호(제5호 제외)에 해당하는 계약은 1인으로부터 받은 견적서에 의할 수 있도록 연결합니다.",
            "- 따라서 1억 원이라는 금액 자체보다 **경쟁입찰을 기다릴 여유가 없는지**, 즉 지연 시 인명·재산 피해가 확대되거나 시설 기능 회복이 곤란한지가 핵심입니다.",
            "",
            "### 법적 근거",
            "1. **수의계약 사유**: 「지방계약법 시행령」 제25조 제1항 제2호. 입찰에 부칠 여유가 없는 긴급복구가 필요한 재난 등 행정안전부령에 따른 재난복구 등의 경우를 확인합니다.",
            "2. **천재지변 등 일반 긴급사유**: 사안에 따라 제25조 제1항 제1호의 천재지변, 감염병, 긴급 행사, 원자재 가격급등 등에 준하는 경우도 함께 검토할 수 있습니다.",
            "3. **1인 견적 근거**: 「지방계약법 시행령」 제30조 제1항 제1호. 제25조 제1항 각 호 중 제5호 소액수의 사유를 제외한 계약은 1인 견적서에 의할 수 있습니다.",
            "4. **금액 기준과의 관계**: 일반 소액공사 기준이나 2천만원 1인 견적 기준을 우회하는 구조가 아니라, 재난·긴급성이라는 별도 수의계약 사유가 선행되어야 합니다.",
            "",
            "### 집행 프로세스",
            "1. **긴급성 판단**: 재난 발생 사실, 긴급복구 지시, 피해 사진·영상, 통행·안전·시설 기능 장애 자료를 확보합니다.",
            "2. **범위 확정**: 전체 복구 물량 중 즉시 조치가 필요한 범위와 추후 경쟁입찰로 돌릴 수 있는 잔여 공사를 구분합니다.",
            "3. **공종과 면허 확인**: 종합·전문·전기·정보통신·소방 등 실제 공종과 법정 면허를 확인합니다. 재난 상황이어도 무면허 시공은 정당화되지 않습니다.",
            "4. **업체 선정**: 부산 업체를 선정하려면 지역 소재 자체가 아니라 현장 접근성, 장비·인력 즉시 투입 가능성, 해당 공종 실적, 안전관리 능력을 사유로 남깁니다.",
            "5. **가격 적정성 검토**: 약식 설계, 표준품셈, 유사 거래가격, 복수 시장조사 견적 또는 사후 정산 조건을 통해 특혜·과다계상 의혹을 줄입니다.",
            "6. **계약 문서화**: 수의계약 사유서에는 `경쟁입찰 소요시간`, `지연 시 피해 확산`, `즉시 조치 범위`, `1인 견적 적용 근거`, `업체 선정 사유`를 분리해 적습니다.",
            "",
            "### 부산업체 활용",
            "- 부산 업체와 계약하려면 `지역업체라서`가 아니라 **긴급 현장 대응성, 장비·인력 보유, 면허, 가격 적정성**을 선정 사유로 쓰는 편이 안전합니다.",
            "- 긴급성이 해소된 후속 복구나 개선공사는 별도 경쟁입찰, 부산 지역제한, 2인 이상 견적 절차로 전환해야 합니다.",
            "",
            "정리하면, 1억 원 공사라도 긴급 재난복구 사유와 `입찰에 부칠 여유가 없음`이 객관적으로 입증되면 1인 견적 수의계약을 검토할 수 있습니다. 다만 즉시 복구에 필요한 범위로 한정하고, 잔여 공사는 정상 경쟁 절차로 분리하는 것이 감사 대응상 안전합니다.",
        ])
    if "학술연구" in q and "용역" in q and any(term in q for term in ("대학", "연구소", "부설연구소")) and "1인" in q:
        ...
        return "\n".join([
            ...
            "- 결론부터 보면, **금액 기준만으로는 부산 지역 대학 부설 연구소와 1인 수의계약을 하기 어렵습니다.**",
            "- 질문 금액이 부가세 제외 추정가격 3천만원이라면 일반 1인 견적 기준인 2천만원을 초과합니다. 또한 대학 부설 연구소나 산학협력단이라는 지위만으로는 여성기업·장애인기업·사회적기업 등 5천만원 1인 견적 특례 대상이 되지 않습니다.",
            "- 다만 해당 연구소만 수행할 수 있는 학술연구 필요성이 객관적으로 입증되면, 금액 기준이 아니라 **「지방계약법 시행령」 제25조 제1항 제4호 차목의 `특정인과의 학술연구 용역계약 필요성`**으로 1인 견적 수의계약을 별도 검토할 수 있습니다.",
            "",
            "### 법적 갈래",
            "1. **일반 1인 견적**: 「지방계약법 시행령」 제30조 제1항 제2호의 2천만원 이하 기준을 초과하므로 이 경로만으로는 어렵습니다.",
            "2. **정책기업 특례**: 청년창업기업·여성기업·장애인기업·사회적기업 등은 5천만원 이하 1인 견적 특례를 볼 수 있지만, 대학 부설 연구소나 산학협력단은 그 지위만으로 해당하지 않습니다. 사업자등록, 법인 성격, 정책기업 확인서를 별도로 확인해야 합니다.",
            "3. **학술연구 소액수의**: 특수한 지식·기술 또는 자격을 요구하는 학술연구용역은 「지방계약법 시행령」 제25조 제1항 제5호 마목의 수의계약 검토 대상이 될 수 있습니다. 하지만 이 경우는 제30조 제1항 제1호의 1인 견적 예외가 아니므로, 원칙적으로 2인 이상 견적 또는 지정정보처리장치 절차를 봐야 합니다.",
            "4. **특정인과의 학술연구**: 반드시 그 대학 연구소여야 하는 경우에는 「지방계약법 시행령」 제25조 제1항 제4호 차목을 검토합니다. 이때는 제30조 제1항 제1호에 따라 1인 견적 가능성이 열리지만, 감사에서 유일성·대체불가능성 입증을 매우 엄격하게 봅니다.",
            "",
            "### 1인 수의가 가능한 쪽으로 가려면",
            "1. **연구 유일성**: 해당 연구소만 보유한 원자료, 장기 추적자료, 선행연구 결과, 특수 장비·데이터·방법론이 과업 핵심과 직접 연결되어야 합니다.",
            "2. **대체기관 비교**: 부산뿐 아니라 전국 대학·연구기관·민간 연구소 중 수행 가능 기관이 있는지 비교표를 만들고, 왜 대체가 곤란한지 적어야 합니다.",
            "3. **과업 관련성**: 연구책임자 이력, 전담 연구진, 최근 실적, 보유 데이터, 지역 현장조사 네트워크가 이번 과업과 직접 맞아야 합니다.",
            "4. **가격 적정성**: 학술연구용역 원가계산, 인건비·경비 산출, 유사 연구용역 거래가격, 협상 내역을 남겨야 합니다.",
            "",
            "### 실무 대안",
            "- 유일성 입증이 약하면 **부산 지역제한 가능성을 검토한 2인 이상 견적**, 제안서 평가, 협상에 의한 계약으로 전환하는 것이 안전합니다.",
            "- 공고나 평가항목에는 `부산 지역 현안 이해도`, `현장조사 수행계획`, `연구진 전문성`, `유사 연구 실적`처럼 과업 품질과 직접 관련된 요소를 넣는 방식이 좋습니다.",
            "- 1인 수의를 선택하려면 수의계약 사유서에 `제25조 제1항 제4호 차목`, `제30조 제1항 제1호`, `대체기관 비교`, `선행연구 연속성`, `가격 적정성`을 분리해 적으세요.",
            "",
            "정리하면, 3천만원 학술연구용역은 **대학 부설 연구소라는 이유만으로 1인 수의계약 대상은 아니며**, 특정 연구소만 수행 가능한 학술연구 필요성을 입증하지 못하면 2인 이상 견적이나 협상계약으로 설계하는 편이 감사 대응상 안전합니다.",
        ])
    if "혁신제품" in q and "수의계약" in q and any(term in q for term in ("금액제한", "금액무관", "제한없이", "제한없이", "한도없이", "가능")):
        return "\n".join([
            "### 혁신제품 수의계약 금액 제한 검토",
            "- 결론부터 보면, **「조달사업법」 제27조 제1항에 따라 지정된 혁신제품은 일반 1인 견적 한도와 별개로 1인 견적 수의계약을 검토할 수 있습니다.**",
            "- 현행 지방계약 기준에서는 「지방계약법 시행령」 제25조 제1항 제8호 다목이 혁신제품 구매 수의계약 근거이고, 제30조 제1항 제1호가 1인 견적 근거입니다.",
            "- 다만 `혁신제품`이라는 명칭만으로 바로 집행하지 말고, 지정 유효기간, 제품명·모델명·규격 일치, 혁신장터·나라장터 등록 상태, 가격 적정성을 먼저 확인해야 합니다.",
            "",
            "### 확인 항목",
            "1. **지정 상태**: 계약체결일 기준 혁신제품 지정이 유효한지 확인합니다.",
            "2. **제품 일치**: 지정된 제품명, 물품식별번호, 모델명, 규격이 구매 규격서와 일치해야 합니다.",
            "3. **구매 경로**: 혁신장터, 나라장터 종합쇼핑몰, 제3자 단가계약 등 실제 납품요구 가능 경로를 확인합니다.",
            "4. **직접생산**: 중소기업자간 경쟁제품 또는 직접생산확인 대상이면 직접생산확인증명서와 실제 공급 주체를 확인합니다.",
            "5. **가격·내부심사**: 법령상 소액 한도와 별개로 예산, 계약심사, 계약심의위원회, 가격 적정성 검토가 필요할 수 있습니다.",
            "6. **부산업체 활용**: 부산 소재는 지역상품 구매 취지와 연결할 수 있지만, 계약 명분은 부산 소재가 아니라 혁신제품 지정에 따른 법정 특례로 세워야 합니다.",
            "",
            "### 정리",
            "- 지정·등록·규격 일치가 확인되면 일반 소액수의 금액 한도 때문에 바로 배제할 사안은 아닙니다.",
            "- 혁신제품은 금액 제한 없이 가능한 것처럼 보이더라도, 고액이면 자체 심사와 감사 소명자료가 훨씬 중요합니다.",
    ...
    if "특허" in q and "수의계약" in q:
        ...
        return "\n".join([
            ...
            f"- 결론부터 보면, **특허를 보유했다는 사실만으로 {amount_label} 수의계약이 바로 가능한 것은 아닙니다.**",
            "- 가능성을 보려면 `부산 업체`가 아니라 **그 특허가 계약 목적 달성에 필수이고, 적절한 대체 기술·제품·공법이 없어 사실상 경쟁이 곤란한지**를 입증해야 합니다.",
            "- 현행 「지방계약법 시행령」 제25조 제1항 제4호 가목은 일반 특허 수의계약 근거가 아니라 공사 하자 책임구분 관련 조항입니다. 특허 수의계약은 계약대상에 따라 아래 목을 구분해 적용해야 합니다.",
            "",
            "### 법적 근거를 나누어 봅니다",
            "1. **특허공법이 필요한 공사**: 「지방계약법 시행령」 제25조 제1항 제4호 마목. 특허공법 등으로 사실상 경쟁이 불가능한 공사인지 봅니다.",
            "2. **특허·실용신안·디자인등록 물품 제조·구매**: 같은 호 사목. 적절한 대용품이나 대체품이 없는 경우인지 봅니다.",
            "3. **생산자 또는 소지자가 1인뿐인 물품**: 같은 호 아목. 다른 물품으로는 사업목적 달성이 어려운지 확인합니다.",
            "4. **특정인의 기술·품질·경험이 필요한 용역 등**: 같은 호 자목. 조사·설계·감리·시설관리 등에서 특정 기술이나 경험이 계약 목적과 직접 연결되는지 봅니다.",
            "5. **1인 견적 근거**: 제25조 제1항 제4호에 해당하면 제30조 제1항 제1호에 따라 1인으로부터 받은 견적서에 의할 수 있습니다. 다만 이는 요건 충족 시의 절차 근거이지, 특허증만으로 자동 성립하는 근거는 아닙니다.",
            "",
            "### 감사 대응 판단 기준",
            "1. **특허 유효성**: 특허등록원부, 존속기간, 권리자, 전용·통상실시권, 계약상대자와 권리자의 관계를 확인합니다.",
            "2. **청구항-과업 대응표**: 특허 청구항과 규격서·시방서·과업 범위를 직접 대응시켜 특허가 과업의 본질적 부분인지 보여야 합니다.",
            "3. **대체가능성 검토서(대체곤란성 입증)**: 타사 제품·공법·기술 3개 이상을 성능, 가격, 공기, 유지관리, 호환성 기준으로 비교해 대체곤란성을 설명합니다.",
            f"4. **특허 적용 범위**: 특허가 일부 기능이나 부품에만 관련되면 {amount_label} 전체를 수의계약하기 어렵습니다. 특허 부분만 분리하거나 기술사용협약·물품공급/기술지원협약 후 일반 경쟁입찰을 검토합니다.",
            "5. **가격 적정성**: 경쟁이 없을수록 원가계산, 유사 거래가격, 조달가격, 협상자료, 사후 정산 조건 등 객관 자료가 필요합니다.",
            f"6. **사전 심의**: {amount_label} 규모라면 계약심사, 기술자문위원회, 공법선정위원회, 계약심의위원회 대상 여부를 내부 기준으로 확인하는 편이 안전합니다.",
            "7. **특허공법 선정절차**: 공사에 적용할 신기술·특허공법이라면 「지방자치단체 입찰 및 계약 집행기준」의 신기술·특허공법 선정기준에 따라 공법선정 안내공고, 제안서 평가, 공법선정위원회, 사용협약 필요 여부를 확인합니다.",
            "",
            "### 부산 업체와 연결할 때",
            "- 부산 업체라는 사정은 현장 대응, 유지보수, 지역경제 효과를 설명하는 보조 사유일 뿐입니다. 수의계약의 법적 근거는 **특허의 필수성·대체불가능성·경쟁곤란성**으로 세워야 합니다.",
            "- 부산의 지형·환경·기존 시설과 해당 특허 기술이 직접 연결된다면 그 기술적 사유를 별도 검토서로 작성하세요.",
            "- 특허 사유가 약하면 협상에 의한 계약, 기술제안입찰, 규격 경쟁, 부산 지역제한 가능성 등 경쟁 절차 안에서 기술성과 현장 대응성을 평가하는 방식이 안전합니다.",
            "",
            f"정리하면, {amount_label} 특허 수의계약은 조건부로 검토할 수 있지만, `특허 보유`와 `부산 업체`만으로는 부족합니다. 품의서에는 적용 목(마목·사목·아목·자목 중 해당 항목), 제30조 제1항 제1호, 청구항-과업 대응표, 대체가능성 검토서, 가격 적정성, 사전 심의 여부를 분리해 남기는 것이 안전합니다.",
        ])
...
def _build_simple_amount_contract_answer(user_message: str, amount_detected) -> str | None:
    ...
    if isinstance(general_one_quote_value, (int, float)) and amount_detected <= general_one_quote_value:
        return "\n".join([
            "### 판단 요약",
            f"- 질문 조건이 **VAT 제외 추정가격 {amount_label} 물품 구매**라면, 확인된 기준상 일반 물품 소액 수의계약과 1인 견적 방식의 검토 범위에 들어갑니다.",
            "- 다만 물품은 바로 업체를 지정하기 전에 **나라장터 종합쇼핑몰/MAS 또는 제3자단가계약 등록 여부**를 먼저 확인하는 것이 안전합니다.",
            "- 쇼핑몰 경로가 없거나 해당 규격·납품조건에 맞지 않으면, 부산 소재 업체와 1인 견적 수의계약을 검토할 수 있습니다. 이때 근거는 `부산업체라서`가 아니라 금액 기준과 수의계약 사유입니다.",
            "",
            "### 근거",
            f"- 일반 물품·용역 수의계약 검토 기준: **추정가격 {general_threshold} 이하**",
            f"- 일반 물품·용역 1인 견적 검토 기준: **추정가격 {one_quote_general} 이하**",
            "- 관련 근거는 「지방계약법 시행령」 제25조 제1항 제5호 가목 및 제30조 제1항의 견적 제출 기준입니다.",
            f"- 여성기업·장애인기업·사회적기업 등 정책기업은 별도 1인 견적 기준인 **추정가격 {one_quote_policy} 이하** 구간을 검토할 수 있으므로, 일반 소액 기준과 혼동하지 마세요.",
            "",
            "### 실무 확인사항",
            "- 질문 금액이 기준 금액과 맞닿아 있으면 추정가격 기준인지, 부가가치세 포함 총액인지, 예정가격·기초금액과 혼동한 것은 아닌지 구분합니다.",
            "- 같은 물품을 기간·부서별로 나누는 구조라면 「지방계약법 시행령」 제77조 제1항의 분할계약 금지 취지상 지적 위험이 있으므로 수요 취합과 산출근거를 남깁니다.",
            "- 부산 지역상품 구매지원 관점에서는 부산 소재 공급업체, 중소기업자간 경쟁제품·직접생산확인 여부, 납품·A/S 가능 지역을 함께 확인하는 편이 좋습니다. 다만 지역상품 구매 지침은 후보 검토와 정책 근거이지 독립적인 수의계약 사유는 아닙니다.",
        ])
