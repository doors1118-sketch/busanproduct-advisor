...
- **답변 Markdown / 후보표 CSV 다운로드:** 시스템이 생성한 마크다운 답변 원문과, 파싱 가능한 표가 존재할 경우에 한정하여 후보표 CSV 다운로드를 지원합니다.
- **민감정보 보호:** 다운로드되는 모든 CSV 및 마크다운 파일은 사전에 사업자등록번호, API 키, 오류 메시지 등이 마스킹 처리된 안전한 상태로 제공됩니다.
