...
```json
{
  "frontend_static_serving": "missing",
  "recommended_change": "Add FastAPI StaticFiles mount for /ui"
}
```
*(※ 이번 단계에서는 코드를 직접 구현하지 않고 보고만 수행합니다. 구현 진행 시 별도의 승인 요청 절차가 필요합니다.)*
...
| frontend static serving 방식 결정 필요 | **대기중** | 승인 시 확정 예정 |
