...
from app.rule_engine.amount_layer import resolve_active_rules
...
def execute_rule_engine(gateway_response: GatewayResponse, request_slots: dict = None) -> DecisionContext:
    ...
    # 4.6. Phase 9.2 Amount Layer (금액 분기 로직)
    amount_result = resolve_active_rules(request_slots)
    ctx.active_rule_ids = amount_result["active_rule_ids"]
    ctx.amount_classification = amount_result["amount_classification"]
    # amount 누락 시 missing_required_slots에 추가
    for slot in amount_result.get("missing_slots", []):
        if slot not in ctx.missing_required_slots:
            ctx.missing_required_slots.append(slot)
