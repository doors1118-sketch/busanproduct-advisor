...
# ── 기본 source map 경로 ──
_DEFAULT_MAP_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
_MERGED_V2_PATH = os.path.join(_DEFAULT_MAP_DIR, "purchase_support_rule_source_map.merged.v2.json")
_MERGED_PATH = os.path.join(_DEFAULT_MAP_DIR, "purchase_support_rule_source_map.merged.json")
_BASE_PATH = os.path.join(_DEFAULT_MAP_DIR, "purchase_support_rule_source_map.json")


def _load_source_map() -> dict:
    """source map을 우선순위에 따라 로드: v2 > merged > base"""
    for path in [_MERGED_V2_PATH, _MERGED_PATH, _BASE_PATH]:
...
def resolve_active_rules(
    ...
    # ── 수의계약 분기 (amount 기반) ──
    # contract_method가 명시적으로 비수의계약이면 수의계약 규칙 비활성화
    # contract_method=None(미지정)이면 금액 기반 양쪽 검토 (사용자가 경로를 특정하지 않았으므로)
    if contract_method == "direct_contract" or (contract_method is None and amount is not None):
        # 소액수의 기준 확인
        threshold = _get_resolved_threshold(
            source_map,
            "R_DIRECT_GENERAL_SMALL_AMOUNT",
            "P_GENERAL_SMALL_DIRECT_CONTRACT_THRESHOLD"
        )
