...
# source_map 로드 (테스트용)
SMAP_PATH = BASE_DIR / "purchase_support_rule_source_map.merged.v2.json"
...
# ── TC2: 물품 + 금액 20000000 (resolved_value=null이므로 unresolved) ──
lines.append("\n--- TC2: goods, amount=20000000, local (numeric unresolved) ---")
result = resolve_active_rules(
    {"contract_object": "goods", "amount": 20000000, "contract_method": "direct_contract"},
    TEST_SOURCE_MAP
)
# resolved_value가 null이므로 양쪽 모두 활성화 (안전 폴백)
check("amount_classification = unresolved (threshold not verified)",
      result["amount_classification"] == "unresolved",
      f"actual: {result['amount_classification']}")
check("Both direct rules active (safe fallback)",
      "R_DIRECT_GENERAL_SMALL_AMOUNT" in result["active_rule_ids"]
      and "R_DIRECT_GENERAL_SMALL_AMOUNT_NOT_PRIMARY" in result["active_rule_ids"])

# ── TC3: 물품 + 금액 200000000 (resolved_value=null이므로 여전히 unresolved) ──
lines.append("\n--- TC3: goods, amount=200000000, local (still unresolved) ---")
result = resolve_active_rules(
    {"contract_object": "goods", "amount": 200000000, "contract_method": "direct_contract"},
    TEST_SOURCE_MAP
)
check("amount_classification = unresolved",
      result["amount_classification"] == "unresolved")
...
check("String amount '20,000,000' works without error",
      result["amount_classification"] == "unresolved")
...
check("Source map has P_GENERAL_SMALL_DIRECT_CONTRACT_THRESHOLD",
      "P_GENERAL_SMALL_DIRECT_CONTRACT_THRESHOLD" in actual_refs,
...
for p in test_map["R_DIRECT_GENERAL_SMALL_AMOUNT"]["numeric_parameters"]:
    if p["parameter_ref"] == "P_GENERAL_SMALL_DIRECT_CONTRACT_THRESHOLD":
...
# ── TC17: contract_method=None + amount → 수의계약 활성화 (양쪽 검토) ──
lines.append("\n--- TC17: contract_method=None + amount → both direct rules ---")
result = resolve_active_rules(
    {"contract_object": "goods", "amount": 30000000},
    TEST_SOURCE_MAP
)
check("contract_method=None+amount: R_DIRECT_GENERAL_SMALL_AMOUNT active",
      "R_DIRECT_GENERAL_SMALL_AMOUNT" in result["active_rule_ids"])
