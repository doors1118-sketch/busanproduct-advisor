...
for s in ['a', 'b', 'c', 'd']:
    ...
    for field in FIELDS:
        val = data.get(field)
        print(f'  {field}: {json.dumps(val, ensure_ascii=False) if isinstance(val, (list, dict)) else val}')

    print()
    print('[Phase 5 Compliance]')
    print(f'  raw_tool_name_leaked_in_answer: {leaked if leaked else "NONE (PASS)"}')
