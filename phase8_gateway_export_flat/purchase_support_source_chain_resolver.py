...
def resolve_sources():
    ...
    for rule in catalog:
        ...
        for term in query_terms:
            # Find matching legal_source (excluding wrong_match, needs_manual_source)
            query = """
            ...
            """
            rows = db.execute(query, (f"%{term}%", f"%{term}%"))
            
            if rows:
                matched_terms.add(term)
                if term in unmatched_terms:
        ...
        if unique_primary:
            ...
            # Use distinct parameters for placeholders
    ...
    with open(REPORT_OUTPUT_PATH, "w", encoding="utf-8") as f:
        ...
        f.write(f"- **Resolved**: {summary['resolved']}\n")
        ...
        for cat, rules in categories.items():
            ...
            f.write("| Rule ID | Name | Status | Primary Sources | Unmatched Terms | Numeric Params |\n")
            f.write("|---------|------|--------|-----------------|-----------------|----------------|\n")
            for r in rules:
                primary_cnt = len(r["primary_source_ids"])
                unmatched = ", ".join(r["unmatched_query_terms"]) if r["unmatched_query_terms"] else "None"
                params = ", ".join([p["parameter_ref"] for p in r["numeric_parameters"]]) if r["numeric_parameters"] else "None"
                
                f.write(f"| `{r['rule_id']}` | {r['display_name']} | `{r['source_chain_status']}` | {primary_cnt} found | {unmatched} | {params} |\n")
            f.write("\n")
