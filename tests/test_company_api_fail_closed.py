...
class TestDefensiveParsing(unittest.TestCase):
    ...
    def test_malformed_rows_skipped(self):
        """candidates에 None, 문자열 등 dict가 아닌 원소가 섞여도 정상 row만 변환."""
        adapter = CompanyAPIAdapter(use_mock=False)
        mock_response = {
            "company_search_status": "success",
            "candidates": [
                None,
                "bad-row",
                {"company_id": "ok1", "company_name": "정상업체", "location": "부산",
                 "license_or_business_type": ["제조업"], "main_products": ["CCTV"]},
                42,
                {"company_id": "ok2", "company_name": "또다른업체", "location": "부산 사하구",
                 "license_or_business_type": ["도매업"], "main_products": []},
            ]
        }
        with patch("app.company_api.search_by_product", return_value=mock_response):
            router_result = MagicMock()
            ...
            result = adapter.resolve(router_result)
            # None, "bad-row", 42는 제거되고 dict 2건만 남아야 함
            self.assertEqual(result.status, "success")
            self.assertEqual(len(result.candidates), 2)
            self.assertEqual(result.candidates[0].company_id, "ok1")
            self.assertEqual(result.candidates[1].company_id, "ok2")

    def test_all_malformed_rows_returns_empty(self):
        """candidates가 전부 malformed면 empty 반환."""
        adapter = CompanyAPIAdapter(use_mock=False)
        mock_response = {
            "company_search_status": "success",
            "candidates": [None, "bad", 123]
        }
        with patch("app.company_api.search_by_product", return_value=mock_response):
            router_result = MagicMock()
            router_result.candidate_lookup_required = True
